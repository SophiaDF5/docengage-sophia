...
### Frontend
- src/pages/CommentGenerator.tsx → "Comments" page (route `/`). Four tabs: Caption and Image draft a comment on a post (pasted text or an uploaded screenshot); Reply to a comment drafts a reply inside the thread instead — pick a post you've scraped and one of its harvested comments (`doc_post_comments`, searchable by name/headline/what they said), or paste a comment in by hand for a post never scraped; Keyword Search Posts lists every lead saved from the Keyword Search page (`doc_contacts` where `source = 'keyword_search'`) and pre-fills the caption-generation call from what that page already captured — the saved author name, profile URL, post URL, and a 500-character post excerpt (editable/extendable before generating, since it's only an excerpt) — no new table or edge function, just reads back what migration 016 already stores. Since the person there is already a saved lead, generating for them updates their existing `doc_contacts` row to `status = 'engaged'` instead of creating a second record the way the other three tabs' Engaged Leads save does — its lead-list query uses `["contacts", orgId, "keyword_search"]` (server-filtered) rather than the bare `["contacts", orgId]` key every other lead page shares, so it can't clobber their full unfiltered list in the cache, while still being a prefix of that key so their status-change invalidations refresh it too. If you fill in Author/commenter Name + LinkedIn URL on the other three tabs, that person is saved into `doc_dm_leads` (Engaged Leads) with `status = 'engaged'` on generation (commenting on — or replying to — their post/comment counts as engaging with them; `saveEngagedLead()` writes the URL to both `links` and `linkedin_profile_url` so later lookups by either column find them) — skipped if a lead with that LinkedIn URL already exists. The Copy button in the output area is what actually approves a draft (`doc_approve_comment`, saving whatever was edited before copying) — see doc_generate_comment's notes below. Recent Comments never deletes anything — no auto-expiry, no per-item delete button — it's meant as a permanent tracker of everything ever drafted (see Rule/decision history in doc_generate_comment).
