...
- supabase/migrations/014_add_tag_to_dm_leads.sql → Adds `tag` text column to doc_dm_leads, mirroring doc_contacts.tag — lets Engaged leads be tagged (e.g. "Invited") too, so Outreach's "Mark Invited" button and tag filter chips work uniformly across all three lead sources
...
- src/pages/Outreach.tsx      → Unified outreach queue merging doc_contacts (scraped + manual) and doc_dm_leads (engaged) into one list. Filtering is layered: Source (Scraped/Engaged/Manual Added, multi-select) narrows which tables/rows are in scope; Status (Pending/Messaged/Engaged) and Tag (built dynamically from whatever tags exist among the source-filtered leads, plus an "Untagged" bucket) further refine within that scope — both use "empty selection = no restriction, click a chip to add one" so they default to showing everything; a text search box matches name/headline. All four combine (AND). Each lead row (both the picker list and the Step 3 review queue, including right next to "Mark as Sent" for convenience) has a "Mark Invited"/"✓ Invited" toggle badge (`src/components/InvitedTagBadge.tsx`) that quick-sets that lead's `tag` to "Invited" (or clears it), writing to whichever table the lead lives in (doc_contacts or doc_dm_leads — both carry `tag` since migration 014) and overwriting any prior tag on that lead; the tag filter chips above pick this up automatically since they're built from live data. Tagging a lead "Invited" does NOT block selecting/messaging it again — the tag is purely for the user's own filtering, not an enforced restriction (tried blocking selection once; reverted per explicit feedback that outreach eligibility shouldn't be tied to this tag). The same `InvitedTagBadge` component is also used on Scraped Leads (Contacts.tsx), Manual Added Leads (ManualLeads.tsx), and Engaged Leads (Leads.tsx) so a lead can be tagged "Invited" from any page, not just Outreach. No upload here anymore — CSV/Excel upload lives on the Manual Added Leads page (see above); this page only composes/sends. Draft messages (bulk or AI-personalized), assisted-send (copies message + opens LinkedIn — never sends automatically, per Out of Scope section). Logs to doc_outreach_messages against whichever source table (`contact_id` or `dm_lead_id`) the lead came from, and writes status back to that same table — see "Status sync" note under doc_outreach_messages below
...
- src/components/InvitedTagBadge.tsx → Shared one-click "Mark Invited"/"✓ Invited" toggle badge — sets/clears a lead's `tag` to "Invited" (see doc_contacts.tag / doc_dm_leads.tag, migrations 013/014). Used on Scraped Leads, Manual Added Leads, Engaged Leads, and Outreach (both the picker list and the review queue) so tagging is consistent everywhere. Same `table`/`queryKey` prop convention as CustomFieldsDialog. Purely a filtering aid — does not restrict messaging/selecting the lead again.
